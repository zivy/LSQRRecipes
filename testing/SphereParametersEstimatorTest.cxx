...
void generateData( unsigned int numPoints, double noiseStandardDeviation,
                   ...
                   std::vector< Point<double,dimension> > &cleanData,
...
bool test3D(std::ostream &out)
{
  ...
  //done generating data
	...
	std::vector<double> sphereParameters;
  double maxDistanceToSphere = 0.5;
  SphereParametersEstimator<3> spEstimator(maxDistanceToSphere);
  ...
  succeedExact = 
    compareAndPrint<3>( "Sphere going through four points [c_x,c_y,c_z,r], no noise:\n\t", 
	    ...
	    //Algebraic least squares estimate of sphere parameters
	spEstimator.setLeastSquaresType(SphereParametersEstimator<3>::ALGEBRAIC);
  ...
  succeedAlgebraic = 
	...
	spEstimator.setLeastSquaresType(SphereParametersEstimator<3>::GEOMETRIC);
	spEstimator.leastSquaresEstimate(pointData, sphereParameters);
  succeedGeometric = 
...
bool test2D(std::ostream &out)
{
  ...
  std::vector<double> trueCircleParameters;
	...
	unsigned int numSamples = 20; //number of points sampled on circle
  ...
  double maxDistanceToCircle = 0.5;
	SphereParametersEstimator<2> cpEstimator(maxDistanceToCircle);
  ...
  succeedExact =
	...
	cpEstimator.setLeastSquaresType(SphereParametersEstimator<2>::ALGEBRAIC);
  ...
  succeedAlgebraic =
    compareAndPrint<2>( "Algebraic least squares circle parameters [c_x,c_y,r]:\n\t", 
		    ...
		    //Geometric least squares estimate of circle parameters
	cpEstimator.setLeastSquaresType(SphereParametersEstimator<2>::GEOMETRIC);
	cpEstimator.leastSquaresEstimate(pointData, circleParameters);
  succeedGeometric =
	...
	pointData.push_back( pnt );
	double sum,err;
	unsigned int numPoints = static_cast<unsigned int>(pointData.size());
...
bool testnD(std::ostream &out)
{
  ...
  double maxDistanceToSphere = 0.5;
	SphereParametersEstimator<dimension> estimator(maxDistanceToSphere);
  ...
  estimator.setLeastSquaresType(SphereParametersEstimator<dimension>::ALGEBRAIC);
  ...
  estimator.setLeastSquaresType(SphereParametersEstimator<dimension>::GEOMETRIC);
  estimator.leastSquaresEstimate(data, sphereParameters);
  succeedGeometric =
...
void generateData(unsigned int numPoints, double noiseStandardDeviation,
                  ...
                  std::vector<double> &sphereParameters)
{
  Vector<double, dimension> tmp, noise;
  Point<double, dimension> sphereCenter, randomPoint;
  double sphereRadius;
