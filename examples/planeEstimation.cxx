 ...
 * inliers/outliers and the estimated plane. The format is compatible with the 
 * viewers available in 
...
void saveOIVFile( std::string &outputFileName,
                  ...
                  lsqrRecipes::PlaneParametersEstimator<dimension> 
                  &parameterEstimator );
/**
...
int main(int argc, char *argv[])
{
  ...
  lsqrRecipes::PlaneParametersEstimator<DIMENSION> planeEstimator(maximalDistanceFromPlane);
  ...
  else
  {
    ...
    saveOIVFile<DIMENSION>( leastSquaresOutputFileName, data, planeParameters, planeEstimator );
  }
  ...
  percentageOfDataUsed = 
    lsqrRecipes::RANSAC< lsqrRecipes::Point<double, DIMENSION>, double>::compute(planeParameters, 
                                                      &planeEstimator, 
                                                      data, 
  ...
  else
  {
    ...
    std::cout<<percentageOfDataUsed<<"\n\n";
       ...
       //save scene file (works only in 3D)
    saveOIVFile<DIMENSION>( ransacOutputFileName, data, planeParameters, planeEstimator );
...
void generateData( unsigned int numInliers, unsigned int numOutliers, 
                   ...
                   std::vector<double> &planeParameters )
{
  lsqrRecipes::Vector<double, dimension> normal, noise, tmp;
  lsqrRecipes::Point<double, dimension> pointOnPlane, randomPoint;
                  ...
                  &parameterEstimator )
{
  ...
  lsqrRecipes::Vector3D tmp, e1, e2, normal;
  ...
  e1 = tmp - (tmp*normal)*normal;
  e1.normalize();
  e2 = crossProduct( normal, e1 );
  
  lsqrRecipes::Point3D p1, p2, p3, p4;
